...
FreeConfig(struct Config *config) {
    ...
    for(int8_t i=config->config.bNumInterfaces - 1;
            i != (uint8_t) -1; --i)
...
//
// returns true and cleanup memory on failure
//
...
GetConfig(struct Config *config, uint8_t *buf, size_t len) {
    ...
    ASSERT(config->interfaces, "calloc(%uhhu,%zu) failed",
    ...
    for(uint8_t i=0; i<numInterfaces; ++i) {
        ...
        if(numEndpoints) {
            ...
            for(uint8_t j=0; i<numEndpoints; ++j) {
                ...
                if(endpoint->bLength != 7 || endpoint->bDescriptorType
                        != USB_DT_ENDPOINT) {
                    ERROR("Found bad USB Endpoint Descriptor");
                    return true;
                }
...
//
static
bool GetDescriptor(struct Descriptor *desc,
        const char *path) {
    ...
    DSPEW("idProduct=%hu", desc->device.idProduct);

    DSPEW("Read %zu bytes", len);
...
struct QsuRtlsdr *qsu_rtlsdr_new(void) {
    ...
    if(GetDescriptor(&rtl->descriptor, path)) {
